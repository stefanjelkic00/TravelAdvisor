...
function Restorani() {
    ...
    const [formData , setFormData] = useState({
        longitude:'' , latitude:'' , adresa:'' , naziv:'' , opis:'' , slikeDestinacije:[] , tipRestorana:''
    });
    ...
    const handleSearch = async()=>{
        await customAxios.get(`http://localhost:8001/api/destinacija/restoranSearch?query=${query}`).then(res => {
            setRestorani(res.data);
    ...
    function openModal(){
      ...
      async function kreirajRestoran(){
        if(formData.longitude == '' || formData.latitude =='' || formData.naziv =='' || formData.adresa =='' ||
            formData.tipRestorana =='' ){
            toast.error("Neophodno je da unesete sve podatke .");
            return;
        }

        await customAxios.post("http://localhost:8001/api/restoran" , {
            longitude: formData.longitude , latitude: formData.latitude , adresa: formData.adresa 
            , naziv: formData.naziv , opis: formData.opis , slikeDestinacije: formData.slikeDestinacije , tipRestorana:formData.tipRestorana
        }).then(res => {
            toast.success("Uspesno ste kreirali restoran");
            const restoran = res.data;
            console.log(restoran);
            setRestorani([...restorani,restoran]);
        }).catch(error => {
            toast.error("Restoran nije kreiran , pokusajte ponovo");
        });
        closeModal();
      }

       const deleteRestoran= async(id) => {
        await customAxios.delete(`http://localhost:8001/api/restoran/${id}`)
            .then(res=> {
                toast.success("Uspesno ste obrisali restoran .");
                const newRestorani = restorani.filter(r => r.id !==id);
                setRestorani(newRestorani);
            }).catch(error =>{
            ...
            <div className='modal-content'>
                ...
                <div className='modal-body'>
                    <form>
                        ...
                        <div>
                            <label htmlFor="tipRestorana">Tip restorana :</label>
                            <br/>
                            <select name="tipRestorana" id="tipRestorana" value={formData.tipRestorana} onChange={handleChange}>
                            ...
                            <option value={"GRCKI"}>GRCKI</option>
                            <option value={"RAZNOVRSNI_TIP"}>RAZNOVRSNI_TIP</option>
                            </select>                   
                        </div>
                    </form>
                </div>
                <div className='modal-footer'>
                    ...
                    <button type='button' className='btn btn-primary' onClick={kreirajRestoran} >Kreiraj</button>
                </div>
            </div>
            </div>
        </div>
        ...
        <div className='d-flex align-items-center justify-content-center'>
            {
                ...
                (
                    <div style={{marginLeft:'150px'}}>
                        ...
                        onClick={openModal}>Dodaj Restoran</button>
                    </div>
                ):(
                    <div>
                        <input type='text' className='me-3' placeholder='Pretraži' value={query} onChange={handleQuery}/>
        ...
        restorani.map(r =>(
            <div className='card me-3 mt-3'style={{width: "350px" , height:"450px"}} key={r.id}>
                ...
                <div className='card-footer'>
                    ...
                    {
                        uloga == "ROLE_ADMIN" && <button className='btn btn-danger btn-sm ms-2' onClick={() => deleteRestoran(r.id)}>Obrisi</button>
                    }
                </div>
            </div>
        ))
